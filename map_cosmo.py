...
class MapCosmo:
    def __init__(self, mappath, feed, jk, splitname, split_no):
        ...
        with h5py.File(mappath, mode="r") as my_file:
            self.x = np.array(
                my_file["x"][:]
            )  # these x, y are are bin centers from mapmaker
            self.y = np.array(my_file["y"][:])
            print(
                "map code", f"/multisplits/{jk}/map_{jk}{split_no}{splitname}", feed - 1
            )
            self.map = np.array(
                my_file[f"/multisplits/{jk}/map_{jk}{split_no}{splitname}"][feed - 1]
            )
            self.rms = np.array(
                my_file[f"/multisplits/{jk}/rms_{jk}{split_no}{splitname}"][feed - 1]
            )
        ...
        sh = self.map.shape
        self.map = self.map.reshape(
            (sh[0], sh[1], sh[2] * sh[3])
        )  # go from 4 sidebands to 1 frequency axis
        ...
        self.w[where] = (
            1 / self.rms[where] ** 2
        )  # weight noise (->pseudo PS), weights outside mask are 0

        meandec = np.mean(self.y)
        self.x = (
            self.x * deg2mpc * np.cos(meandec * np.pi / 180)
        )  # account for the effect that 'circles of latitude' around poles are smaller than equator
    ...
    def coadd_feed_maps(self, map_file, which_split, jk):
        ...
        for i in range(19):
            map_single_feed = np.array(
                map_file["/jackknives/map_" + jk][which_split, i]
            )
            rms_single_feed = np.array(
                map_file["/jackknives/rms_" + jk][which_split, i]
            )
