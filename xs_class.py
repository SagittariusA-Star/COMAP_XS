...
class CrossSpectrum_nmaps:
    def __init__(self, name_of_my_map, jk=False, feed1=None, feed2=None, n_of_splits=2):
        ...
        self.name_of_map = (
            name_of_my_map  # the names schould indicate which map and feed we take
        )
        self.names = []
        self.maps = []
        ...
        for u in range(self.how_many_combinations):
            ...
            my_map_split_1 = map_cosmo.MapCosmo(
                name_of_my_map, feed1, jk[0], jk[1], current_combo[0]
            )
            name1 += self.feed_name1

            name2 = jk[0] + str(current_combo[1]) + jk[1]
            my_map_split_2 = map_cosmo.MapCosmo(
                name_of_my_map, feed2, jk[0], jk[1], current_combo[1]
            )
    ...
    # PLOT XS
    def plot_xs(
        self, k_array, xs_array, rms_sig_array, rms_mean_array, save=False, outdir=""
    ):
        for index in range(self.how_many_combinations):
            k = k_array[index]
            xs = xs_array[index]
            ...
            ax1.errorbar(
                k, k * xs, k * rms_sig, fmt="o", label=r"$k\tilde{C}_{data}(k)$"
            )  # added k*
    ...
    def get_tf(self):
        ...
        self.transfer_sim_2D = scipy.interpolate.interp2d(
        ...
        self.transfer = scipy.interpolate.interp1d(
        ...
        self.transfer_filt_2D = scipy.interpolate.interp2d(
            k_perp_filt, k_par_filt, TF_filtering_2D
        )
