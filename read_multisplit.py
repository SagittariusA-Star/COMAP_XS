...
def read_map(
    ...
    if (
        len(feed_and_test) != 0
    ):  # if some test variables are simultaneously feed-feed variables
        for test_variable in feed_and_test:
            ...
            # shp = map_split.shape
            how_many_twos = (
                len(control_variables) + 1
            )  # how many parts to reshape the map with respect to splits
            # new_shape = []
            # for i in range(how_many_twos):
            #    new_shape.append(2) #because we split in 2 - needs to be changed if more splits are implemented
            # new_shape.append(shp[1]) #feed
            ...
            # slc = [slice(None)]*len(new_shape) #includes all elements
            for i in range(
                len(all_different_possibilities)
    ...
    if len(feed_and_control) != 0:  # if some feed-feed variables are control variables
        for ff_variable in feed_and_control:
            for test_variable in test_variables:
                ...
                how_many_twos = (
                    len(all_variables) - len(test_variables) + 1
                )  # how to reshape the map with respect to splits
                new_shape = []
                for i in range(how_many_twos):
                    new_shape.append(2)
                new_shape.append(shp[1])  # feed
                ...
                how_many_to_combine = (
                    len(split_names) - 1
                )  # test variable + all control variables, except for the ff_variable
                all_different_possibilities = list(
                    itr.product(range(2), repeat=how_many_to_combine)
                )  # find all the combinations of 'how_many_to_combine' 0s and 1s
                index_of_ff_variable = split_names.index(ff_variable)
                ...
                for i in range(
                    ...
                    maps_created.append(
                        name
                    )  # add the name of the current map to the list
                    print("Creating HDF5 file for the map " + name + ".")
                    """tools.ensure_dir_exists('split_maps/' + outdir)
               #outname = 'split_maps/' + name
               outname = 'split_maps/' + outdir + '/' + name
...
def null_test_subtract(maps_created, test_variables, field, outdir):
   ...
   """
    ...
    for i in range(len(test_variables)):
        ...
        new_map12 = write_map_created(
            mapfile1, map12, rms12, test_variable, "0", field, outdir
        )
        new_map34 = write_map_created(
            mapfile1, map34, rms34, test_variable, "1", field, outdir
        )
...
def read_field_jklist(mappath):
    ...
    jk_list = (
        "/mn/stornext/d16/cmbco/comap/protodir/auxiliary/jk_list_" + last_part + ".txt"
    )
