...
# (important with jk_list - keep 3s first, then 2s, and two first lines irrelevant) MAYBE TRY TO MAKE IT MORE GENERAL
...
def read_map_ft(mappath,field, control_variables, test_variables, feed_feed_variables, all_variables, feed_and_test, feed_and_control):
   ...
   return maps_created
'''
def read_map(mappath,field, control_variables, test_variables, feed_feed_variables, all_variables, feed_and_test):
   print ('STAGE 2/4: Splitting the map into subsets with different split combinations.')
   input_map = h5py.File(mappath, 'r')
...
'''
control_variables, test_variables, feed_feed_variables, all_variables, feed_and_test, feed_and_control = read_jk('/mn/stornext/d16/cmbco/comap/protodir/maps/co2_map_signal.h5')
